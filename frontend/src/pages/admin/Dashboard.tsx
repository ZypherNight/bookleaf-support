                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2 mb-1.5">
                          ...
                          <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border shadow-sm flex items-center ${ticket.status === 'Resolved' || ticket.status === 'Closed'
      ...
      <div className="fixed bottom-8 right-8 z-50 flex flex-col gap-3 pointer-events-none">
        {notifications.map(notif => (
          <div key={notif.id} className="pointer-events-auto bg-white rounded-2xl p-4 border border-slate-200 shadow-2xl shadow-slate-900/10 flex items-start gap-4 animate-in slide-in-from-right-8 fade-in duration-300 w-80">
            <div className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center ${notif.type === 'new' ? 'bg-brand-50 text-brand-600' : 'bg-amber-50 text-amber-600'
              }`}>
